...
class CompanyOrganizerManager extends Component
{
    ...
    {
        $this->event = Event::with('companyOrganizators')->findOrFail($this->eventId);
    }
    ...
    {
        $usedIds = $this->event->companyOrganizators->pluck('id')->toArray();
...
//        $this->dispatchBrowserEvent('notify', ['message' => 'Организатор успешно добавлен']);
    ...
    {
        $this->event->companyOrganizators()->detach($organizerId);
...
//        $this->dispatch('notify', ['message' => 'Организатор удалён']);
    ...
    {
        return view('livewire.event.company-organizer-manager');
    }
}
