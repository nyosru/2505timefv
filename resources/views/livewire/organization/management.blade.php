...
{{--                                [ 'link'=>'no', 'name'=>'Счета']--}}
    ...
    <div class="px-2">
        ...
        <div class="flex justify-between mb-3">
            <input type="text" wire:model.debounce.500ms="search" placeholder="Поиск по названию"
                   class="border p-2 rounded w-1/3"/>
            <button wire:click="create" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Добавить
                организацию
            </button>
        </div>
                ...
                <tr>
                    ...
                    <td class="border border-gray-300 p-2">
                        @if($org->website)
                            <a href="{{ $org->website }}" target="_blank"
                               class="text-blue-600 underline">{{ $org->website }}</a>
                        @else
                            -
                        @endif
                    </td>
                    <td class="border border-gray-300 p-2 whitespace-nowrap">
                        <button wire:click="edit({{ $org->id }})"
                                class="px-2 py-1 bg-yellow-400 text-white rounded hover:bg-yellow-500 mr-2">
                            Редактировать
                        </button>
                        <button wire:click="delete({{ $org->id }})"
                                class="px-2 py-1 bg-red-600 text-white rounded hover:bg-red-700"
                                onclick="confirm('Удалить организацию?') || event.stopImmediatePropagation()">Удалить
                        </button>
                    </td>
                </tr>
            @empty
                ...
                <div class="bg-white rounded shadow-lg p-6 w-full max-w-xl relative">
                    ...
                    <form wire:submit.prevent="save" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="city_id" wire:model.defer="city_id"
                                    class="w-full border rounded px-3 py-2 @error('city_id') border-red-600 @enderror">
                                <option value="">-- не выбран --</option>
                                @foreach($cities as $city)
                                    <option value="{{ $city->id }}">{{ $city->name }} ({{ $city->country->name ?? 'x' }}
                                        )
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input id="address" type="text" wire:model.defer="address"
                                   class="w-full border rounded px-3 py-2 @error('address') border-red-600 @enderror"/>
                            @error('address')
                            <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input id="website" type="url" wire:model.defer="website"
                                   class="w-full border rounded px-3 py-2 @error('website') border-red-600 @enderror"/>
                            @error('website')
                            <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                        </div>
                        ...
                        <div class="flex justify-end space-x-2">
                            <button type="button" wire:click="closeModal"
                                    class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Отмена
                            </button>
                            <button type="submit"
                                    class="px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">{{ $organizerId ? 'Сохранить' : 'Добавить' }}</button>
                        </div>
                    </form>

                    <!-- Кнопка закрытия модального окна -->
                    <button type="button" wire:click="closeModal"
                            class="absolute top-2 right-2 text-gray-600 hover:text-gray-800 text-xl font-bold">&times;
                    </button>
                </div>
            </div>
        @endif
