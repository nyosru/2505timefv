...
{{--            {{ $event->venue }}--}}
    ...
    <div class="flex flex-col mt-[5px]">
        <h4 class="leading-[30px] font-bold">@if(!empty($event->events_date_finished)) Период проведения @else Дата проведения @endif            @if($event->event_date->isFuture())
                <span class="bg-red-300 p-1 rounded-md">скоро</span> @endif
        </h4>
        <a class="text-[14px] leading-[25px] pb-[10px] text-[#003493] underline" href="#">
{{--            20/12/2025 - 24/12/2026--}}
            {{ $event->event_date->format('d.m.Y') }}
                    @if(!empty($event->events_date_finished) ) - {{ $event->events_date_finished->format('d.m.Y')  }}@endif
        </a>
    </div>
    @endif
